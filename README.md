...
A robust, server-side rendered web application designed to browse automotive catalogs, compare vehicle specifications, and provide personalized recommendations based on user behavior.
...
* **Behavioral Recommendation Engine:** A custom algorithm that analyzes session history to determine brand affinity and category preferences. It employs a 4-slot strategy:
    ...
    2.  *Brand Loyalty:* A different model from the user's most visited manufacturer.
    3.  *Competitor Comparison:* A model from the second most visited manufacturer.
    4.  *Discovery:* A popular vehicle from the user's preferred category.
...
The data source is located in the separate `carapi` directory. Please refer to the `README.md` within that folder for detailed configuration options.
